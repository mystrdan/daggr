...
create index if not exists market_updates_published_at_idx
  on public.market_updates (published_at desc);

create index if not exists market_updates_source_id_idx
  on public.market_updates (source_id);

alter table public.market_updates enable row level security;

create policy "market_updates_public_read"
